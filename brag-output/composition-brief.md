...
- Duration: 27.2 seconds (as built, v4)
...
Two additions: the 2.2× unusual band on the Nucor card, and a 1080x1920 cut. See `brag-plan.md`
→ "v4" for the rationale; this is the verification record.

**Landscape** — `composition/`, 1920x1080, 27.2s, 816 frames, H.264 + AAC, `--quality delivery`.
`check`: **0 errors**, 2 warnings (the descent seam at t=13.72–13.85), 1 info (the card body
mid-expansion, which `overflow: hidden` is there to clip).

**Vertical** — `composition-vertical/`, 1080x1920, 27.2s, 816 frames. Generated by
`build-vertical.py`; it is not hand-edited and should not be. `check`: **0 errors**, 6 warnings
(the same descent seam plus the scene 3→4 crossfade), 1 info (the same body expansion).

Band geometry derives from the real NUE snapshot row — typical 2.84%, today −6.32%, multiple
2.2× — mapped across a −7.5%…+7.5% track at build time in both cuts. The inversion bar geometry
likewise recomputes for the narrower vertical track from the same `backtest.json` returns, so
neither cut carries a hand-placed pixel.

Posters are the assembled Nucor card at 18.5s in each aspect, baked as frame 0.

Contrast reports `0/0 text checks` in both, as before — it samples nothing rather than passing
